...
fn main() -> Result<()> {
    let vibe_dir = dirs::home_dir().context("No home directory")?.join(".vibe");
    ...
    for (project_id_blob, project_name) in &projects {
        let tasks_dir = vibe_dir.join("projects").join(project_name).join("tasks");
        ...
        println!("  Project '{}': {} tasks", project_name, tasks.len());
